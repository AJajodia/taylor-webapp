...
taylor_album_focus <- taylor_all_songs %>%
  ...
  mutate(album_length_min = sum(duration_ms, na.rm = T)/60000,
         ...
         album_explicit = sum(explicit, na.rm=TRUE)
         ) %>%
    ungroup()

taylor_all_songs %>% group_by(album_name) %>% count(explicit)

as.integer(as.logical(taylor_all_songs$explicit))

((taylor_all_songs %>% count(explicit)) %>% filter(explicit == "TRUE"))[[2]]
